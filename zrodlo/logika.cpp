...
void Logika::uwzglWejscie() {
	...
	if(stanKlawiatura[DIK_W] & 0x80) {
		obiektWybrany->wykonajZdarzRuch(XMVectorSet(+0.0f, +0.002f, +0.0f, +0.0f));
	}
	if(stanKlawiatura[DIK_S] & 0x80) {
		obiektWybrany->wykonajZdarzRuch(XMVectorSet(+0.0f, -0.002f, +0.0f, +0.0f));
	}
	if(stanKlawiatura[DIK_A] & 0x80) {
		obiektWybrany->wykonajZdarzRuch(XMVectorSet(-0.002f, +0.0f, +0.0f, +0.0f));
	}
	...
	if(stanMysz.lX != 0) {
		...
		obiektKursor->wykonajZdarzRuch(XMVectorSet(p, +0.0f, +0.0f, +0.0f));
	}
	if(stanMysz.lY != 0) {
